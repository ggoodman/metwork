module.exports =
  ...
  """
  controller: ["$scope", "$state", "oauth", "users", "session", "notifier", ($scope, $state, oauth, users, session, notifier) ->
    ...
    for identity in session.user.identities
      for field, value of identity when value
        serviceName = oauth.getService(identity.service)?.name
    ...
    $scope.updateUser = (userInfo) ->
      session.user[field] = value for field, value of userInfo
      session.user.put().then ->
        notifier.success "User updated"
  ]